...
const App = () => {
  ...
  const handleLogin = async (username, password) => {
    try {
      ...
      try {
        ...
        console.log("Login POST raw response:", response.data);

        // Always return the POST result if we got a valid JSON response
        ...
        console.warn("POST failed, attempting GET fallback...", postErr.message);
      }

      // GET Fallback
      const getUrl = `${AUTH_API}?action=login&username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`;
      const response = await axios.get(getUrl);
      console.log("Login GET raw response:", response.data);
  ...
  const handleRegister = async (username, email, password) => {
    try {
      console.log("Attempting registration via Sheets:", { username, email });
      const AUTH_API = import.meta.env.VITE_AUTH_API_URL;
      ...
      try {
        const response = await axios({
          method: 'post',
          url: AUTH_API,
          data: JSON.stringify({ action: 'register', username, email, password }),
          headers: { 'Content-Type': 'text/plain;charset=utf-8' },
          timeout: 10000
        });

        console.log("Register POST raw response:", response.data);

        // Always return the POST result if we got a valid JSON response
        if (response.data && typeof response.data === 'object') {
          if (response.data.success) return { success: true };
          return { success: false, message: response.data.message || "Registration failed (backend returned no message)" };
        }
      } catch (postErr) {
        console.warn("Register POST failed, trying GET fallback:", postErr.message);
      }

      // GET Fallback
      const getUrl = `${AUTH_API}?action=register&username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`;
      const response = await axios.get(getUrl);
      console.log("Register GET raw response:", response.data);

      if (response.data && response.data.success) {
        return { success: true };
      } else {
        return { success: false, message: response.data?.message || "Registration failed (no message from server)" };
      }
    } catch (err) {
      console.error("Sheets Registration error:", err);
