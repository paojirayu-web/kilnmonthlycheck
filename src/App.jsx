...
const App = () => {
  ...
  const handleLogin = async (username, password) => {
    try {
      ...
      // Method 1: Try POST with text/plain (Simple Request)
      try {
      ...
      // Method 2: GET Fallback (Usually bypasses all 405/CORS issues with GAS)
      ...
      return { success: false, message: `Login Error: ${err.message}. Please check if the Google Script is deployed as 'Anyone'.` };
    }
  };
  ...
  const handleRegister = async (username, email, password) => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // Return exactly what the backend says, or a specific fallback
        const errMsg = response.data?.message || (response.data ? "Backend error (no message)" : "No response from backend");
